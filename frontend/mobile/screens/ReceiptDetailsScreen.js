...
export default function ReceiptDetailsScreen({ route, navigation }) {
  ...
  const isMock = !receiptData && !receiptId;
  const initialInfo = receiptData?.data?.receipt_info || receiptData?.receipt_info || {};
  const initialItems = isMock ? ITEMS : initialInfo.items ? [...initialInfo.items] : [];

  const [merchantName, setMerchantName] = useState(
    isMock ? "Reliance Fresh" : initialInfo.merchant_name || "Unknown Store",
  );
  const [totalAmount, setTotalAmount] = useState(
    isMock ? 1245.6 : initialInfo.total_amount || 0.0,
  );
  const [dateStr, setDateStr] = useState(
    isMock ? "20 July 2025" : initialInfo.date || "Unknown Date",
  );
      ...
      const fetchReceipt = async () => {
        ...
        try {
          ...
          if (response.ok && data.status === "success") {
            const info = data.data.receipt_info;
            setMerchantName(info.merchant_name || "Unknown Store");
            setTotalAmount(info.total_amount || 0.0);
            setDateStr(info.date || "Unknown Date");
    ...
    try {
      ...
      const payload = {
        ...
        items: extractedItems.map((it) => ({
          name: it.name,
          matched_food_id: it.food_id || it.matched_product_id,
      ...
      const response = await fetch(`${API_BASE_URL}/receipts/save`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        >
          ...
          <View style={styles.tableCard}>
            ...
            {extractedItems.map((item, i) => {
              ...
              const calories =
                item.nutrition?.calories_kcal ?? item.calories ?? "-";
              const protein =
              ...
              return (
                ...
                >
                  <View style={styles.colItem}>
                    ...
                    {!isEditing &&
                      ...
                      item.matched_name !== item.name && (
                        <Text style={styles.matchedNameText} numberOfLines={1}>
                          {item.matched_name}
                        </Text>
                      )}
                  </View>
                  {isEditing ? (
                    <TextInput
                      style={[styles.colQty, styles.tableInput]}
                      value={String(item.quantity || item.qty || 1)}
                      onChangeText={(val) => updateItem("qty", val)}
                    />
                  ) : (
                  ...
                  {isEditing ? (
                    <TextInput
                      style={[styles.colRate, styles.tableInput]}
                      value={
                        item.unit_price != null
                      ...
                      onChangeText={(val) => updateItem("rate", val)}
                    />
                  ) : (
                  ...
                  {isEditing ? (
                    <TextInput
                      style={[styles.colPrice, styles.tableInput]}
                      value={
                        item.total_price != null
                      ...
                      onChangeText={(val) => updateItem("price", val)}
                    />
                  ) : (
                  ...
                  {foodDetailsLoaded && (
                    <Text style={[styles.colCal, styles.cellText]}>
                      {calories}
                    </Text>
                  )}
                  {foodDetailsLoaded && (
                    <Text style={[styles.colProt, styles.cellText]}>
                      {protein}
                    </Text>
                  )}
                </TouchableOpacity>
          ...
          <View style={styles.summaryCard}>
            <InfoRow
              label="Total Items"
              value={extractedItems.length.toString()}
            />
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
            onPress={() => navigation.push('AIInsights', { receiptId })}
          >
...
const styles = StyleSheet.create({
  ...
  tableInput: {
    ...
    color: COLORS.primary,
    borderBottomWidth: 1,
    borderBottomColor: "rgba(153,8,8,0.2)",
    padding: 0,
    margin: 0,
  },
  matchedNameText: {
